    ...
    if (newsettings.api.client.allow.server.create == true) {
      queue.addJob(async (cb) => {
        ...
        if (
          ...
          // Enforce one-server-per-user policy
          if (servers2 >= 1) {
            cb();
          ...
          if (!isNaN(ram) && !isNaN(disk) && !isNaN(cpu)) {
            ...
            if (specs["environment"] && typeof specs["environment"] === "object") {
              // For Paper/Minecraft servers, ensure VANILLA_VERSION is set
              if (specs["environment"]["SERVER_JARFILE"] && !specs["environment"]["VANILLA_VERSION"]) {
                specs["environment"]["VANILLA_VERSION"] = specs["environment"]["VANILLA_VERSION"] || "latest";
              }
              // Set default values for missing environment variables
              for (let key in specs["environment"]) {
                if (specs["environment"][key] === undefined || specs["environment"][key] === null) {
                  specs["environment"][key] = "";
                }
              }
            }
